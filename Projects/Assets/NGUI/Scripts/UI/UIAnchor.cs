...
public class UIAnchor : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (wc != null)
		{
			UIStretch us = wc.GetComponent<UIStretch>();
			if (us != null)
			{
				if (!us.started)
					return;
		...
		{
			UIStretch us = container.GetComponent<UIStretch>();
			if (us != null)
			{
				if (!us.started)
					return;
		...
		if (uiRoot == null)
			return;
		float r = useCamera ? (uiRoot.fitWidth ? (float)Screen.width / uiRoot.manualWidth : (float)Screen.height / uiRoot.manualHeight) : 1;
        if (side == Side.Center)
        {
            v.x += (pixelOffset.x * r + relativeOffset.x * (width - 2 * Mathf.Abs(pixelOffset.x) * r));
            v.y += (pixelOffset.y * r + relativeOffset.y * (height - 2 * Mathf.Abs(pixelOffset.y) * r)); 
        }
        else
        {
            v.x += (pixelOffset.x * r + relativeOffset.x * width);
            v.y += (pixelOffset.y * r + relativeOffset.y * height); 
        }
		...
		if (useCamera)
		{
            ...
            //v.z = uiCamera.WorldToScreenPoint(mTrans.position).z;//remed by kjh
		...
		else
		{
            ...
            //v.z = mTrans.position.z;//remed by kjh
		}
