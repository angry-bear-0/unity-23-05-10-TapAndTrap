...
public class UIScrollView : MonoBehaviour
{
    ...
    /// <summary>
    ///  how much scale from finger moving.
    /// </summary>
    ...
    protected int mScrollPos = 0; //added by kjh
    public int scrollPos { get { return mScrollPos; } }
		...
		if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
				...
				else
				{
					...
					else MoveAbsolute(offset * dragScale); //kjh
				}
	...
	void LateUpdate ()
	{
        mScrollPos = (int)mPanel.finalClipRegion.x; //added by kjh
