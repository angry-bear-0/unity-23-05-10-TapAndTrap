...
public class UIPopupList : UIWidgetContainer
{
	...
	public virtual void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
		{
			...
			float labelHeight = activeFontSize * activeFontScale * fontScale;
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				...
				lbl.transform.localScale = new Vector3(fontScale, fontScale, 1);
				...
				y -= lineHeight;
				x = Mathf.Max(x, lbl.printedSize.x * fontScale);
			...
			for (int i = 0, imax = labels.Count; i < imax; ++i)
			{
				...
				if (bc != null)
				{
					...
					bc.center = bcCenter;
					bc.size = bcSize * 1 / fontScale; //kjh
				}
				else
				{
			...
			int lblWidth = Mathf.RoundToInt(x / fontScale + 0.5f);
