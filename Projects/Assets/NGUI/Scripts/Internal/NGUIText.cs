...
static public class NGUIText
{
	...
	static public void Update (bool request)
	{
		...
		finalSpacingX = spacingX * fontScale - Mathf.Clamp01(1 - fontScale) / 0.2f; //kjhkjh//finalSpacingX = spacingX * fontScale;
		finalLineHeight = (fontSize + spacingY) * 1; //kjhkjh //finalLineHeight = (fontSize + spacingY) * fontScale;
	...
	//kjhkjh
	static public string SpaceDigitString(string s)
	{
		string sr = string.Empty;
		for (int i = 0; i < s.Length; i++)
		{
			int di = s.Length - i;
			if (s.Length != di && (di % 3) == 0)
				sr += ",";
			sr += s.Substring(i, 1);
		}
		return sr;
	}
	static public string SpaceDigitString(int n)
	{
		string s = n.ToString();
		return SpaceDigitString(s);
	}
	...
	static public GlyphInfo GetGlyph (int ch, int prev, float fontScale = 1f)
	{
		if (bitmapFont != null)
		{
			...
			if (bmg != null)
			{
				...
				if (fontScale != 1f)
				{
					glyph.v0.x *= fontScale; //kjhkjh //glyph.v0 *= fontScale;
					glyph.v1.x *= fontScale; //kjhkjh //glyph.v1 *= fontScale; 
		...
		else if (dynamicFont != null)
		{
			if (dynamicFont.GetCharacterInfo((char)ch, out mTempChar, finalSize, fontStyle))
			{
				...
				if (pd != 1f)
				{
					glyph.v0.x *= pd; //kjhkjh //glyph.v0 *= pd; 
					glyph.v1.x *= pd; //kjhkjh //glyph.v1 *= pd; 
					glyph.advance *= pd;
				}
				return glyph;
			}
	...
	static public void Print (string text, List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol != null)
			{
				...
				v0y = v1y - symbol.height;//kjhkjh //v0y = v1y - symbol.height * fontScale; 
				var w = symbol.advance * scale;
