...
#region Fill Functions
	...
	protected void SlicedFill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols, ref Vector4 v, ref Vector4 u, ref Color gc)
	{
		...
		if (mFlip == Flip.Horizontally || mFlip == Flip.Both)
		{
			...
			mTempPos[1].x = mTempPos[0].x + ((v.z - v.x) > (br.z + br.x) ? br.z : Mathf.Min(br.z, (v.z - v.x) / 2)); //kjh
			//mTempPos[2].x = mTempPos[3].x - br.x;
			mTempPos[2].x = mTempPos[3].x - ((v.z - v.x) > (br.z + br.x) ? br.x : Mathf.Min(br.x, (v.z - v.x) / 2)); //kjh
		...
		else
		{
			...
			mTempPos[1].x = mTempPos[0].x + ((v.z - v.x) > (br.z + br.x) ? br.x : Mathf.Min(br.x, (v.z - v.x) / 2)); //kjh
			//mTempPos[2].x = mTempPos[3].x - br.z;
			mTempPos[2].x = mTempPos[3].x - ((v.z - v.x) > (br.z + br.x) ? br.z : Mathf.Min(br.z, (v.z - v.x) / 2)); //kjh
